...
*/
	QR(const TNT::Matrix<Real> &A)		/* constructor */
	{
      QR_=A;
      m = A.dim(1);
      n = A.dim(2);
...
*/
   ...
   */
   ...
   TNT::Matrix<Real> getHouseholder (void)  const
   {
   	  TNT::Matrix<Real> H(m,n);
   ...
   */
	...
	TNT::Matrix<Real> getR() const
	{
      TNT::Matrix<Real> R(n,n);
      for (int i = 0; i < n; i++) {
         for (int j = 0; j < n; j++) {
            if (i < j) {
               R[i][j] = QR_[i][j];
   ...
   */
	...
	TNT::Matrix<Real> getQ() const
	{
	  int i=0, j=0, k=0;

	  TNT::Matrix<Real> Q(m,n);
      for (k = n-1; k >= 0; k--) {
         for (i = 0; i < m; i++) {
            Q[i][k] = 0.0;
         }
   ...
   */
   ...
   TNT::Matrix<Real> solve(const TNT::Matrix<Real> &B) const
   {
   	  if (B.dim1() != m)		/* arrays must be conformant */
	  	return TNT::Matrix<Real>(0,0);
