...
namespace
{
								 ...
								 Implicit* imp, POINT* p)
  {
    ...
    while (1) {
      ...
      if (i++ == RES) return;
      if ((imp->proc(gmVector3(p->x, p->y, p->z))) > 0.0)
  ...
  void vnormal (Implicit* imp, POINT* point, POINT* n, float delta)
  {
    double f = imp->proc(gmVector3(point->x, point->y, point->z));
    n->x = (float)(imp->proc(gmVector3(point->x+delta, point->y, point->z))-f);
    n->y = (float)(imp->proc(gmVector3(point->x, point->y+delta, point->z))-f);
    n->z = (float)(imp->proc(gmVector3(point->x, point->y, point->z+delta))-f);
    f = sqrt(n->x*n->x + n->y*n->y + n->z*n->z);
    if (fabs(f) >gmEPSILON) {n->x /= (float)f; n->y /= (float)f; n->z /= (float)f;}
  }
  ...
  {	   /* parameters, function, storage */
    ...
    Implicit* imp;	   /* implicit surface function */
  ...
  CORNER* PROCESS::setcorner (int i, int j, int k)
  {
    ...
    c->i = i; c->x = (float)(start.x+((float)i-.5)*size);
    c->j = j; c->y = (float)(start.y+((float)j-.5)*size);
    c->k = k; c->z = (float)(start.z+((float)k-.5)*size);
    ...
    c->value = (float)(imp->proc(gmVector3(c->x, c->y, c->z)));
  ...
  TEST PROCESS::find (int sign, float x, float y, float z)
  {
    ...
    for (i = 0; i < 10000; i++) {
      ...
      test.value = (float)(imp->proc(gmVector3(test.p.x, test.p.y, test.p.z)));
  ...
  int PROCESS::vertid (CORNER* c1, CORNER* c2)
  {
    ...
    converge(&a, &b, c1->value, imp, &v); /* position */
    vnormal(imp, &v, &n, delta);			   /* normal */
  ...
  PROCESS::PROCESS(Implicit* _imp,
									 float _size, float _delta, 
									 int _bounds, 
									 vector<VERTEX>& _gvertices,
									 vector<NORMAL>& _gnormals,
									 vector<TRIANGLE>& _gtriangles):
    imp(_imp), size(_size), delta(_delta), bounds(_bounds),
  ...
  void PROCESS::march(int mode, float x, float y, float z)
  {
    ...
    converge(&in.p, &out.p, in.value, imp, &start);
...
void Polygonizer::march(bool tetra, float x, float y, float z)
{
	...
	gnormals.clear();
	gtriangles.clear();
	PROCESS p(imp, size, size/(float)(RES*RES), bounds, 
