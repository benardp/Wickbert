//*****************************************************************************
//	CSGIntersection.h: Implementation file for the CSGIntersection class.
//	Author: Xinlai Ni
//	1/13/2006
//*****************************************************************************
...
REGISTER_IMPLICIT(CSGIntersection, "BinaryOp:CSGIntersection");


void CSGIntersection::procq(const gmVector3 & x, double* q)
{
...
double CSGIntersection::proc(const gmVector3 & x)
{
  if ((m_f!=NULL) && (m_g!=NULL))
...
gmVector3 CSGIntersection::grad(const gmVector3 & x)
{
  if ((m_f!=NULL) && (m_g!=NULL))
    {
      double fx = m_f->proc(x); 
...
gmMatrix3 CSGIntersection::hess(const gmVector3 & x)
{
  if ((m_f!=NULL) && (m_g!=NULL))
    {
      double fx = m_f->proc(x); 
