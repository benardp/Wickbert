...
class ScalarLattice{
 ...
 public:
  ...
  virtual bool ContainsPoint(const Vector3d& v) const = 0;
  virtual AABoundingBox GetAABBox() const = 0;

  Cell ToCell(const Vector3d& v) const;

  virtual scalar Value(const Cell& c) const = 0;

  virtual Vector3d Normal(const Cell& c) const;
  virtual scalar   Upwind(const Cell & c, const Vector3d& direction) const;
  virtual Vector3d UpwindNormal(const Cell & c, const Vector3d& direction) const;
  virtual scalar MeanCurvature(const Cell& c) const;

  // Use trilinear interpolation to find desired values
  virtual scalar   InterpValue(const  Vector3d& v) const;
  virtual Vector3d InterpNormal(const Vector3d& v) const;
  virtual scalar   InterpMeanCurvature(const Vector3d& v) const;
