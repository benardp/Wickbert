/*
 *  ShaderDeCarloSuggestiveContour.cpp
 *  gps
 *
...
#include <vector>
#include <set>
#include "ShaderDeCarloSuggestiveContour.h"
...
REGISTER_PARTICLESTUFF(ShaderDeCarloSuggestiveContour,"Shader:ShaderDeCarloSuggestiveContour");

ShaderDeCarloSuggestiveContour::ShaderDeCarloSuggestiveContour(Particles *ps)
:ShaderContour(ps,std::string("ShaderDeCarloSuggestiveContour"))
{
...
void ShaderDeCarloSuggestiveContour::drawPost(void)
{
...
void ShaderDeCarloSuggestiveContour::findTangent(const unsigned int i)
{
	gmVector3 tangent; 

	gmVector3 v = *(view->getCameraPosition()) - position->getPosition(i); //view vector
	if(useRadCurvNumerator)
		impInt->getImplicit()->gradNumeratorOfRadialCurvature(tangent, position->getPosition(i), v);
	else
		impInt->getImplicit()->gradRadialCurvature(tangent, position->getPosition(i), v);

	tangent = cross(impInt->grad(i),tangent);
...
double ShaderDeCarloSuggestiveContour::computeDerivative(const int i)
{
	gmVector3 suggNormal;
	gmVector3 v = *(view->getCameraPosition()) - position->getPosition(i); //view vector
	if(useRadCurvNumerator)
		impInt->getImplicit()->gradNumeratorOfRadialCurvature(suggNormal, position->getPosition(i), v);
	else
		impInt->getImplicit()->gradRadialCurvature(suggNormal, position->getPosition(i), v);

	return dot(suggNormal,v);
}
