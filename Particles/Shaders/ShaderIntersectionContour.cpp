 ...
 * @file ShaderIntersectionContour.cpp
 * @date 6/25/2005
 * @author Matei N. Stroila
 * @remarks
 */
#include "ShaderIntersectionContour.h"
...
REGISTER_PARTICLESTUFF(ShaderIntersectionContour,"Shader:ShaderIntersectionContour");

ShaderIntersectionContour::ShaderIntersectionContour(Particles *ps)
:ShaderContourSimple(ps,std::string("ShaderIntersectionContour"))
{
	// The particle tangent is already defined in ShaderContourSimple
	// new Attached<ParticleVector>(this,&tangentAttr);

	new Attached<ImplicitInterrogator>(this,&secondsurf);
}

void ShaderIntersectionContour::attachAttributes()
{
...
void ShaderIntersectionContour::findTangent(const unsigned int i)
{
	gmVector3 posi = position->getPosition(i);

	gmVector3 ni = impInt->grad(i);
	ni.normalize();

	gmVector3 mi = secondsurf->grad(i);
	mi.normalize();

	gmVector3 tangent = cross(ni, mi);
...
void ShaderIntersectionContour::drawPost(void)
{
