	...
	: ParticleBehavior(ps, name)
{
	...
	new PSParamBool(this,&useContours,false,"useContours","useContours:","Use the calculated connectivity to determine");
}
	...
	for (unsigned int i=0; i<oldSize; i++) 
	{
		...
		if (
			...
			(
				...
				||
				//if the chain we created is too long, we have a potential problem and subdivide
				(	useContours && 
					(velocity->v[contours->chains[i].n1].length()<gamma*rep_data->r[contours->chains[i].n1])
					&& (impint->proc(contours->chains[i].n1)<fabs(fateval))
					&&
					(	(
						(position->getPosition(i)
						-
						position->getPosition(contours->chains[i].n1)
						).length()
						> rep_data->sigma_hat
						)
						||
						...
						(position->getPosition(i)
						-
						position->getPosition(contours->chains[i].n2)
						).length()
						> rep_data->sigma_hat
						)
					)
					//but only if the probability tells us too...
					&&
					(psRandom() 
					<  0.001*nu*
					max(						
