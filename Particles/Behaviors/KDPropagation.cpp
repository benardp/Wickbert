...
//	s += gaussfac*imp_int->kg(position->x[i]);
//	s += meanfac*imp_int->km(position->x[i]);
	...
	if( loaded ) {
		...
		//s *= -1;
		gmVector3 norm = p_orient->getNormal(i);
		s = v[0] * norm[0] + v[1] * norm[1] + v[2] * norm[2];
		/*double n = sqrt(v[0] * v[0] + v[1] * v[1] + v[2] * v[2]);
		double n2 = norm.length();
		double cosphi = u / (n * n2);
		s *= cosphi;*/
	}
	return s;
}
