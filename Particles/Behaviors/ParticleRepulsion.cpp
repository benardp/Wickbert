/**
 * Implementation of the repulsion force.
 * @file ParticleRepulsion.cpp
 * @date August 31, 2001
 * @author Ed Bachta
 */
...
REGISTER_PARTICLESTUFF(ParticleRepulsion,"Behavior:ParticleRepulsion");
	...
	: ParticleBehavior(ps, std::string("ParticleRepulsion"))
{
	...
	new Attached<ParticleLocality>(this,&p_locality,"ParticleLocality",
...
void ParticleRepulsion::applyForce() {
	...
	for (i = 0; i < ps->size(); ++i) {
		...
		for (nbr = neighbors.begin(); nbr != neighbors.end(); nbr++) {
			...
			//ps->v[j] += -(sig2/(rep_data->r[j]*rep_data->r[j]))*rij*Eij;
		...
		// you can uncomment it if particles are ill-behaved.
		// rep_data->r[i] = fabs(rep_data->r[i]);
		if (rep_data->r[i] > rep_data->sigma_max) {
			rep_data->dr[i] = 0;
